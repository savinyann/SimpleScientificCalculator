...
angle.set("rad")
fromBase.set("dec")
fromBaseNumber.set(10)
toBase.set("dec")

def updateFromBase(value):
	if(fromBase.get() == "bin"):
		fromBaseNumber.set(2)
...
def convertValue(valueINT, valueDEC):
	if(toBase.get() == "dec"):
		INT = valueINT
		DEC = valueDEC
	if(toBase.get() == "bin"):
		INT = bin(valueINT)[2:] if (int(valueINT) >= 0) else "-" + bin(-int(valueINT))[2:]
		DEC = bin(valueDEC)[2:] if (int(valueDEC) != None) else None
	if(toBase.get() == "oct"):
		INT = oct(valueINT)[2:] if (int(valueINT) >= 0) else "-" + oct(-int(valueINT))[2:]
		DEC = oct(valueDEC)[2:] if (int(valueDEC) != None) else None
	if(toBase.get() == "hex"):
		INT = hex(valueINT)[2:] if (int(valueINT) >= 0) else "-" + hex(-int(valueINT))[2:]
		DEC = hex(valueDEC)[2:] if (int(valueDEC) != None) else None
	if(valueDEC == '0'):
		return(str(INT))
	return(str(INT + "." + DEC))

def updatetext(value):
	global Seconde, dot, calc_ended, displayValue, valueINT, valueDEC
	valueINT = value.split('.')[0]
	if(len(value.split('.')) == 2):
		valueDEC = value.split('.')[1]
	displayValue.set(convertValue(valueINT, valueDEC).upper())
...
def reset_func():
	global value1, value2, value3, value, valueINT, valueDEC, dot, calc_ended
	value1 = None
	value2 = None
	valueINT = None
	valueDEC = '0'
...
def softReset():
	global value, valueINT, valueDEC
	value = '0'
	valueINT = '0'
	valueDEC = '0'
...
def equ_func():
	global value1, value2, value3, value, dot, calc_ended, valueINT, valueDEC
	if(value1 != None and value2 != None):
		unit_digit = 0
		if(value2 == "+"):
			while(len(value1[1]) != len(valueDEC)):
				value1[1] = value1[1]+'0' if(len(value1[1]) < len(valueDEC)) else value1[1]
				valueDEC = valueDEC+'0' if(len(value1[1]) > len(valueDEC)) else valueDEC
			valueDEC = str(int(value1[1], fromBaseNumber.get()) + int(valueDEC, fromBaseNumber.get()))

			if(len(valueDEC) > len(value1[1])):
				unit_digit = len(valueDEC) - len(value1[1])
				unit = valueDEC[0:unit_digit]
				valueDEC = valueDEC[unit_digit:]
			valueINT = str(int(value1[0], fromBaseNumber.get()) + int(valueINT, fromBaseNumber.get()) + unit_digit)
			value = valueINT + "." + valueDEC if(valueDEC != 0) else valueINT
		...
		if(value2 == "-"):
			...
			if(int(valueDEC) < 0):
				print(valueDEC)
				print(type(valueDEC))
				valueDEC = str(int(valueDEC) + math.pow(10, len(valueDEC)))
				unit_digit = -1
			valueINT = str(int(value1[0], fromBaseNumber.get()) - int(valueINT, fromBaseNumber.get()) - unit_digit)
			value = valueINT + '.' + valueDEC if(valueDEC != 0) else valueINT
		if(value2  == "*"):
			value = str(int(value1, fromBaseNumber.get()) * int(value, fromBaseNumber.get()))
		if(value2  == "/"):
			if(int(value) == 0):
				error()
			else:
...
def add_func():
	global value1, value2, value3, dot, valueINT, valueDEC
	if(value1 != None):
		equ_func()
	value1 = [valueINT, valueDEC]
	value2 = "+"
	softReset()
	dot = False
